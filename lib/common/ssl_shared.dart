import 'dart:convert';
import 'dart:developer';
import 'package:http/http.dart' as http;
import 'package:http/io_client.dart';
import 'dart:io';
import 'package:flutter/services.dart';

class SSLShared {
  static Future<HttpClient> cstmHttpClnt({
    bool tstIsMode = false,
  }) async {
    SecurityContext ctx = SecurityContext(withTrustedRoots: false);
    try {
      List<int> thebytes = [];

      if (tstIsMode) {
        thebytes = utf8.encode(_certifStr);
      } else {
        thebytes = (await rootBundle.load('certificates/certificates.pem')).buffer.asUint8List();
      }
      ctx.setTrustedCertificatesBytes(thebytes);
    } on TlsException catch (e) {
      if (e.osError?.message != null && e.osError!.message.contains('CERT_ALREADY_IN_HASH_TABLE')) {
        log('createHttpClient() - cert already trusted! Skipping.');
      } else {
        log('createHttpClient().setTrustedCertificateBytes EXCEPTION: $e');
        rethrow;
      }
    } catch (e) {
      log('unexpected error $e');
      rethrow;
    }
    HttpClient httpClnt = HttpClient(context: ctx);
    httpClnt.badCertificateCallback = (X509Certificate cert, String host, int port) => false;

    return httpClnt;
  }

  static Future<http.Client> createLEClient({bool tstIsMode = false}) async {
    IOClient clnt = IOClient(await SSLShared.cstmHttpClnt(tstIsMode: tstIsMode));
    return clnt;
  }
}

const _certifStr = """"-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----""";
